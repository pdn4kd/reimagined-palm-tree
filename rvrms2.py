...
def extinction(lam):
	...
	return (0.09 + (3080.0/lam)**4)

def rvcalc(Teff, FeH, logg, vsini, theta_rot, rstar, dstar, airmass, exptime, efficiency, area, R, gain, read_noise, dark_current, n_pix, lammin, lammax, dlam):
	# Need to fix to make less unit dependent
	lam_max = lammax.to(u.angstrom).value
	lam_min = lammin.to(u.angstrom).value
	deltalam = dlam.to(u.angstrom).value
	
	BeattyWaves = np.arange((lam_min+deltalam/2), (lam_max+deltalam/2), deltalam)
	...
	# sum of flux(lam)*deltalam(lam)/1e8 == total flux (in power/area) emitted. 
	...
	for lam in np.arange(0, len(BeattyWaves)):
		pix = n_pix*deltalam*R/I_0[lam][0]
		...
		I_0[lam][4] = I_0[lam][3]**2 * pix / ((c.c/(u.km/u.s)).si * deltalam/I_0[lam][0])
...
if __name__ == '__main__':
	...
	lam_min = 3800 * u.angstrom
	lam_max = 6800 * u.angstrom
	deltalam = 100 * u.angstrom
	...
	print(rvcalc(Teff, FeH, logg, vsini, theta_rot, rstar, dstar, airmass, exptime, efficiency, area, R, gain, read_noise, dark_current, n_pix, lam_min, lam_max, deltalam))
